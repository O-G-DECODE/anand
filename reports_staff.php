...
<body>
    ...
    <form action="event_report.php" method="post">
        ...
        <?php
// Fetch event names from the event table for the staff member
$stmt = $conn->prepare("SELECT event_id, name FROM event WHERE staff_id = ?");
if ($stmt) {
    $stmt->bind_param("i", $_SESSION['staff_id']); // Use the session's staff_id
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Check if the event_id exists in the request table with approve > 0
        $checkStmt = $conn->prepare("SELECT COUNT(*) as count FROM request WHERE event_id = ? AND approve > 0");
        if ($checkStmt) {
            ...
            if ($checkRow['count'] > 0) {
                // If there are approved requests, display the event name
                echo "<option value='" . htmlspecialchars($event_name) . "'>" . htmlspecialchars($event_name) . "</option>";
            }
            $checkStmt->close();
        }
    }
